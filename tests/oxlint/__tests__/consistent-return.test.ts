...
const RULE_ID = 'typescript/consistent-return';

describe(RULE_ID, () => {
  it('valid', async () => {
    const diagnostics = await runOxlint(
      [path.join(import.meta.dirname, 'consistent-return.valid.ts')],
      { typeAware: true }
    );
  ...
  it('invalid', async () => {
    const diagnostics = await runOxlint(
      [path.join(import.meta.dirname, 'consistent-return.invalid.ts')],
      { typeAware: true }
    );
