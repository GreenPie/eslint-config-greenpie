...
const RULE_ID = 'node/no-mixed-requires';

describe(RULE_ID, () => {
  it('valid', async () => {
    const diagnostics = await runOxlint([
      path.join(import.meta.dirname, 'no-mixed-requires.valid.ts')
    ]);
  ...
  it('invalid', async () => {
    const diagnostics = await runOxlint([
      path.join(import.meta.dirname, 'no-mixed-requires.invalid.ts')
    ]);
