...
const RULE_ID = 'typescript/strict-void-return';

describe(RULE_ID, () => {
  it('valid', async () => {
    const diagnostics = await runOxlint(
      [path.join(import.meta.dirname, 'strict-void-return.valid.ts')],
      { typeAware: true }
    );
  ...
  it('invalid', async () => {
    const diagnostics = await runOxlint(
      [path.join(import.meta.dirname, 'strict-void-return.invalid.ts')],
      { typeAware: true }
    );
