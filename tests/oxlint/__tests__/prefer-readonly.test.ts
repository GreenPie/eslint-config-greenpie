...
const RULE_ID = 'typescript/prefer-readonly';

describe(RULE_ID, () => {
  it('valid', async () => {
    const diagnostics = await runOxlint(
      [path.join(import.meta.dirname, 'prefer-readonly.valid.ts')],
      { typeAware: true }
    );
  ...
  it('invalid', async () => {
    const diagnostics = await runOxlint(
      [path.join(import.meta.dirname, 'prefer-readonly.invalid.ts')],
      { typeAware: true }
    );
