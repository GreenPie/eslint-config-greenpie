...
const RULE_ID = 'node/handle-callback-err';

describe(RULE_ID, () => {
  it('valid', async () => {
    const diagnostics = await runOxlint([
      path.join(import.meta.dirname, 'handle-callback-err.valid.ts')
    ]);
  ...
  it('invalid', async () => {
    const diagnostics = await runOxlint([
      path.join(import.meta.dirname, 'handle-callback-err.invalid.ts')
    ]);
